...
- [ ] 5.6 ~~Find every fragment / query selecting `Post.appUser`~~ **DEFERRED** until task group 1 (WP plugin) lands. Reason: the rewrite to `Post.author.node.user.appUser` requires `User.appUser` to exist in the federated schema, which only happens after the WP federation plugin is updated and the WP container is rebuilt. Current `Post.appUser` keeps working in the meantime via the (still-in-place) `PostAppUserResolver`.
